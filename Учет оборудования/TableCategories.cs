...
{
	public partial class TableCategories : Form
	{
		...
		private void TableCategories_Load(object sender, EventArgs e)
		{
		...
		{
			for (int col = 0; col < категорииDataGridView.ColumnCount; col++)
			for (int row = 0; row < категорииDataGridView.RowCount - 1; row++)
				категорииDataGridView[col, row].Style.BackColor = Color.White;
			for (int col = 0; col < категорииDataGridView.ColumnCount; col++)
			for (int row = 0; row < категорииDataGridView.RowCount - 1; row++)
				if (категорииDataGridView[col, row].Value.ToString().IndexOf(textBox1.Text, StringComparison.OrdinalIgnoreCase) >= 0)
				{
					for (int Q = 0; Q < категорииDataGridView.ColumnCount; Q++)
						категорииDataGridView[Q, row].Style.BackColor = Color.Khaki;
				}
			for (int col = 0; col < категорииDataGridView.ColumnCount; col++)
			for (int row = 0; row < категорииDataGridView.RowCount - 1; row++)
				if (категорииDataGridView[col, row].Value.ToString().IndexOf(textBox1.Text, StringComparison.OrdinalIgnoreCase) >= 0)
					категорииDataGridView[col, row].Style.BackColor = Color.Orange;
			textBox1.Clear();
		}
	...
	ref oMissing, ref oMissing);
		...
		private void button3_Click(object sender, EventArgs e)
		{
			...
			sfd.FileName = "Категории.docx";

			if (sfd.ShowDialog() == DialogResult.OK)
			{
				string ter = "Категории";
				exportToWord(категорииDataGridView, sfd.FileName, ter);
			}
		}
		...
		private void button4_Click(object sender, EventArgs e)
		{
			...
			exportToExcel(категорииDataGridView);
		}
	}
}
