...
{
	public partial class TableWarehouse : Form
	{
		...
		private void TableWarehouse_Load(object sender, EventArgs e)
		{
		...
		{
			for (int col = 0; col < складDataGridView.ColumnCount; col++)
			for (int row = 0; row < складDataGridView.RowCount - 1; row++)
				складDataGridView[col, row].Style.BackColor = Color.White;
			for (int col = 0; col < складDataGridView.ColumnCount; col++)
			for (int row = 0; row < складDataGridView.RowCount - 1; row++)
				if (складDataGridView[col, row].Value.ToString().IndexOf(textBox1.Text, StringComparison.OrdinalIgnoreCase) >= 0)
				{
					for (int Q = 0; Q < складDataGridView.ColumnCount; Q++)
						складDataGridView[Q, row].Style.BackColor = Color.Khaki;
				}
			for (int col = 0; col < складDataGridView.ColumnCount; col++)
			for (int row = 0; row < складDataGridView.RowCount - 1; row++)
				if (складDataGridView[col, row].Value.ToString().IndexOf(textBox1.Text, StringComparison.OrdinalIgnoreCase) >= 0)
					складDataGridView[col, row].Style.BackColor = Color.Orange;
			textBox1.Clear();
		}
	...
	ref oMissing, ref oMissing);
		...
		private void button3_Click(object sender, EventArgs e)
		{
			...
			sfd.FileName = "Склады.docx";

			if (sfd.ShowDialog() == DialogResult.OK)
			{
				string ter = "Склады";
				exportToWord(складDataGridView, sfd.FileName, ter);
			}
		}
		...
		{
			exportToExcel(складDataGridView);
